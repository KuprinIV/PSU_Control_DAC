...
{
	uint8_t res = 0;
	if(HAL_I2C_IsDeviceReady(&hi2c1, (dev_address<<1), 3, 100) == HAL_OK)
	{
		res = 1;
	}
	return res;
...
void MCP4725_writeDAC_register(uint8_t dev_address, uint16_t dac_value, uint8_t is_save_in_eeprom)
{
	...
	HAL_I2C_Master_Transmit(&hi2c1, (dev_address<<1), data, sizeof(data), 1000);
}
...
uint16_t MCP4725_readDAC_register_and_EEPROM(uint8_t dev_address, DacDataType data_type)
{
	...
	if(HAL_I2C_Master_Receive(&hi2c1, (dev_address<<1), data, sizeof(data), 1000) == HAL_OK)
	{
		status = data[0];
